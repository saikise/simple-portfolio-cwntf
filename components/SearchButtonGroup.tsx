import {
  SEARCH_BUTTON_GROUP_BOOK_NUGGETS_ICON,
  SEARCH_BUTTON_GROUP_CODE_CRAFTS_ICON,
  SEARCH_BUTTON_GROUP_THOUGHTS_ICON,
} from "@/constants/icons";
import Link from "next/link";
import { SERIES } from "../constants/data";

interface SearchButtonGroupProps {
  keyword: string;
  seriesCode: string;
}

export default function SearchButtonGroup({
  keyword,
  seriesCode,
}: SearchButtonGroupProps) {
  return (
    <div className="inline-flex rounded-md shadow-sm" role="group">
      {SERIES.map((s, i) => {
        // Purpose of addRoundedCorner is to determine  if rounded corners should be added to the style of the Link
        const addRoundedCorner =
          i === 0
            ? "rounded-l-lg"
            : i === SERIES.length - 1
            ? "rounded-r-md"
            : "";

        // Purpose of isActive is to determine if the Link is active
        const isActive =
          (s.code === "all" && !seriesCode) || seriesCode === s.code;
        // Purpose of url is to get the url of the Link
        const url =
          s.code === "all"
            ? `/search?q=${keyword}`
            : `/search?q=${keyword}&t=${s.code}`;
        // Purpose of svg is to determine what svg icon the Link will use
        const svg =
          s.title === "Code Crafts" ? (
            SEARCH_BUTTON_GROUP_CODE_CRAFTS_ICON
          ) : s.title === "Book Nuggets" ? (
            SEARCH_BUTTON_GROUP_BOOK_NUGGETS_ICON
          ) : s.title === "Thoughts" ? (
            SEARCH_BUTTON_GROUP_THOUGHTS_ICON
          ) : (
            <></>
          );

        return (
          <Link
            key={`search-button-group-${s.code}`}
            href={url}
            className={`inline-flex items-center border border-gray-200 px-4 py-2 text-xs ${addRoundedCorner} hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:text-blue-700 focus:ring-2 focus:ring-blue-700 dark:border-gray-600 dark:text-white dark:hover:bg-gray-600 dark:hover:text-white dark:focus:text-white dark:focus:ring-blue-500 ${
              !keyword && "pointer-events-none cursor-not-allowed opacity-50"
            } ${
              isActive && !!keyword
                ? "bg-gray-100 text-blue-700 dark:bg-gray-600"
                : "bg-white text-gray-900 dark:bg-gray-700"
            }`}
          >
            {svg}
            {s.title}
          </Link>
        );
      })}
    </div>
  );
}
